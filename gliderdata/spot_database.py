...
class SpotDatabase:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if cursor.rowcount > 0:
                    conn.commit()
                    logger.debug(f"Inserted position for {position.asset_id} at {position.timestamp}")
                    return True
                else:
                    logger.debug(f"Position already exists for {position.asset_id} at {position.timestamp}")
                    return False
    ...
    def insert_positions(self, positions: List[SpotPosition]) -> int:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                logger.info(f"Inserted {inserted_count} new positions out of {len(positions)} total")
    ...
    def get_latest_position(self, asset_id: Optional[str] = None) -> Optional[Dict[str, Any]]:
        """
    ...
    def get_positions_since(self, since: datetime, asset_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """
    ...
    def get_asset_ids(self) -> List[str]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute('SELECT DISTINCT asset_id FROM positions ORDER BY asset_id')
    ...
    def get_position_count(self, asset_id: Optional[str] = None) -> int:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if asset_id:
                    cursor.execute('SELECT COUNT(*) FROM positions WHERE asset_id = ?', (asset_id,))
                else:
    ...
    def cleanup_old_positions(self, days_to_keep: int = 30) -> int:
        ...
        try:
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                logger.info(f"Cleaned up {deleted_count} old positions (older than {days_to_keep} days)")
    ...
    def get_database_stats(self) -> Dict[str, Any]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Get unique assets
                cursor.execute('SELECT COUNT(DISTINCT asset_id) as unique_assets FROM positions')
                unique_assets = cursor.fetchone()['unique_assets']
                
                # Get date range
                cursor.execute('SELECT MIN(timestamp) as earliest, MAX(timestamp) as latest FROM positions')
