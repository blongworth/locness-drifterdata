...
def setup_logging(level: str = "INFO"):
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler('spot_tracker.log'),
...
def cmd_status(args):
    ...
    for asset in stats['asset_breakdown']:
        print(f"  {asset['asset_id']}: {asset['count']} positions "
...
Examples:
  ...
  %(prog)s config                   # Create sample config file
    ...
    # Test command
    test_parser = subparsers.add_parser('test', help='Test API connection and database setup')
    test_parser.set_defaults(func=cmd_test)
    
    # Collect command
    collect_parser = subparsers.add_parser('collect', help='Run a single data collection cycle')
    ...
    # Status command
    status_parser = subparsers.add_parser('status', help='Show database status and statistics')
